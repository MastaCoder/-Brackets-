...
export async function joinTournament(user, tid) {
	...
	await addLog(user.username, `Joining tourney: ${tournament.name}`);
...
export async function removeUserFromTournament(req, userToRemove) {
	...
	const actionMessage =
		req.user.username === userToRemove
			? `Leaving ${tournament.name}`
			: `Kicked ${userToRemove} from ${tournament.name}`;
...
export async function updateTournamentInfo(req) {
	...
	await addLog(req.user.username, `Updating ${tournament.name} info`);
	await tournament.save();
	tournament.userTeam = null;
...
export async function updateTournamentStatus(req) {
	...
	if (tournament.status === 1)
		actionMessage = `Starting tournament: ${tournament.name}`;
	if (tournament.status === 2)
		actionMessage = `Ending tournament: ${tournament.name}`;
	await addLog(req.user.username, actionMessage);
	await tournament.save();
	tournament.userTeam = null;
