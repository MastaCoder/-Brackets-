...
export default function TournamentViewPage(props) {
	...
	useEffect(() => {
		axios
			...
			.catch((err) => setTournament(false));
	}, [props.match.params.id]);

	const onJoinTournament = async () => {
		try {
			const res = await axios.post(
				`/api/tournaments/join/${props.match.params.id}`
			);
	...
	const onKickTournament = async (userToRemove) => {
		...
		try {
			const res = await axios.post(
				`/api/tournaments/kick/${props.match.params.id}`,
				{
	...
	const onTeamNameChange = async (newName) => {
		if (newName === tournament.userTeam) return;

		try {
			const res = await axios.patch(
				`/api/tournaments/teams/changename/${props.match.params.id}`,
				{
	...
	const onTeamKick = async (kickedUser) => {
		...
		try {
			const res = await axios.post(
				`/api/tournaments/teams/kick/${props.match.params.id}`,
				{
					groupName: tournament.userTeam,
	...
	const onRegenerateLink = async () => {
		try {
			const res = await axios.post(
				`/api/tournaments/regenerate/${props.match.params.id}`
			);
	...
	const onTeamJoin = async (groupName) => {
		try {
			const res = await axios.post(
				`/api/tournaments/teams/join/${props.match.params.id}`,
				{
					groupName,
				}
			);
	...
	const onTournamentUpdate = async (description, pub) => {
		try {
			const res = await axios.patch(
				`/api/tournaments/updateinfo/${props.match.params.id}`,
				{
					public: pub,
	...
	const onNextStage = async () => {
		try {
			const res = await axios.post(
				`/api/tournaments/nextstatus/${props.match.params.id}`
			);
			setTournament(res.data.tournament);
		} catch (err) {
			console.log(err);
	...
	const onNextBracket = async (proceedingTeams) => {
		try {
			const res = await axios.post(
				`/api/tournaments/nextbracket/${props.match.params.id}`,
				{
